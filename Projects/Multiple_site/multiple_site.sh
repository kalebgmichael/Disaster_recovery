...
WORDPRESS_CONTAINER_NAME_one="wordpress1"
WORDPRESS_CONTAINER_NAME_two="wordpress2"
MASTER_PORT=3308
WORKER_PORT=3307
WORDPRESS_PORT_1=8799
WORDPRESS_PORT_2=8798
...
if ! is_container_running $MASTER_CONTAINER_NAME; then
  ...
  WORDPRESS_DB_HOST=$WORKER_CONTAINER_NAME
    ...
    if is_container_exists $WORDPRESS_CONTAINER_NAME; then
        ...
        docker run -d \
        --name $WORDPRESS_CONTAINER_NAME_one \
        --network $NETWORK_NAME \
        -e WORDPRESS_DB_HOST=$WORDPRESS_DB_HOST:3306 \
        -e WORDPRESS_DB_NAME=$MYSQL_DATABASE \
        -e WORDPRESS_DB_USER=$MYSQL_USER \
        -e WORDPRESS_DB_PASSWORD=$MYSQL_PASSWORD \
        -p $WORDPRESS_PORT_1:80 \
        wordpress:latest

        echo "WordPress setup complete. Access it at http://localhost:$WORDPRESS_PORT_1"

        ##
          # Run the WordPress container if both DB are up and running and the wordpress container does not exist
        echo "Running WordPress container for the first time with master DB..."
        docker run -d \
        --name $WORDPRESS_CONTAINER_NAME_two \
        --network $NETWORK_NAME \
        -e WORDPRESS_DB_HOST=$WORDPRESS_DB_HOST:3306 \
        -e WORDPRESS_DB_NAME=$MYSQL_DATABASE \
        -e WORDPRESS_DB_USER=$MYSQL_USER \
        -e WORDPRESS_DB_PASSWORD=$MYSQL_PASSWORD \
        -p $WORDPRESS_PORT_2:80 \
        wordpress:latest

        echo "WordPress setup complete. Access it at http://localhost:$WORDPRESS_PORT_2"
    fi
fi
